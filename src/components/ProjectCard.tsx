...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <div className="group flex flex-col justify-between overflow-hidden rounded-2xl border border-[#E7E2DA] bg-[#FFFFFF] p-4 sm:p-5 shadow-xs transition-colors duration-200 hover:border-[#DDD7CD] hover:shadow-sm">
      <div className="space-y-4">
          ...
          <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 opacity-0 transition-opacity duration-200 group-hover:opacity-100 z-10">
            {project.githubUrl && (
              <a
                ...
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#E7E2DA] bg-[#FAF8F5]/90 text-[#1A1815] shadow-xs backdrop-blur-md transition-colors hover:border-[#DDD7CD] hover:bg-[#FFFFFF]"
              >
        ...
        <div className="flex items-baseline justify-between gap-2">
          <h3 className="text-lg sm:text-xl font-bold tracking-tight text-[#1A1815]">
            {project.title}
          </h3>
          <span className="shrink-0 font-mono text-xs font-medium text-[#75726B]">
            {project.date}
          </span>
        </div>
